...
You can check examples on [Storybook](https://main--64b35eb2b079785924e82a2f.chromatic.com/)
...
The `<Root>` component requires some React element to be passed as a prop (i.e. `element={<MyComponent />}`). By default, the content of the element's source custom component is considered to be of type 'text', which may suffer a page break. If that is not the case, the value 'block' should be passed to the prop `content` to indicate that the content of the custom component is not divisible.

The `css/base` stylesheet should be imported to apply the basic styles required by the library. The `css/print` stylesheet may be used to apply print styles.
...
- If the content of the component is of type 'text', the registered node must have a `line-height` CSS property that corresponds to that of the content.
...
- In addition to `margin` you can create space between content with the `gap` CSS property. For that the you need to give
