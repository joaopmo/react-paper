...
React-Paper is a library for allocating custom components content into paper sheets like layouts. New pages are generated automatically to handle overflowing content. It also exports CSS for printing the pages. 
...
React-Paper was developed for a resume builder kind of web app, where it was generally used to generate 1 or 2 pages documents. It may have poor performance in larger scales.
...
All the components related to the React-Paper library must be wrapped within a `<Paper />` component to ensure they work as expected. The `<Paper />` component must have one or more `<Column />` components as children. Each `<Column />` can have one or more `<Root />` components as children.

The `<Root />` component require some custom component to be passed as a prop (i.e. `element={<MyComponent />}`). By default, the content of the custom component is considered to be of type 'text', which may suffer a page break. The value 'block' should be passed to the prop `content` to inform that the content of the custom component is not divisible.

The `css/base` stylesheet should be imported to apply the basics styles required by the library. The `css/print` stylesheet may be used to apply print styles.
...
```
One of the DOM elements returned by the component passed to `<Root />` must be registered into the `useRegister()` hook. You can have many nested elements, but there are some rules that need to be followed for everything to work as expected:

- If the content of the component is of type 'text', the registered element must have a `line-height` CSS property that corresponds to that of the content.
- The only element with `margin-top`, `margin-bottom`, `border-top`, `border-bottom`, `padding-top` or `padding-bottom` CSS properties should be the one registered.
...
The rules for registering elements make it hard to create complex layouts. For that you can use the `<Level />` and `<Node />` components. The `<Level />` component is used to wrap one or more `<Node />` components. The `<Node />` component is similar to the `<Root />` component, but it can be nested to create more complex layouts. 
...
One of the elements in the custom component passed to `<Root />` needs to be registered like in the previous examples. But this time, the content  consists of three other custom components that are passed to `<Node />` components. The first two have content of type 'text' that possibly have distinct `font` related CSS properties. The last one is an image with content of type 'block'. All then are separated by 10px by the `row-gap` CSS property, in addition to spacing that may be created by `margin` related CSS properties of the registered elements.
...
- `parallel`: A boolean indicating if the React Elements passed to the `<Node />` children are rendered vertically or horizontally in respect to one another. By default, React-Paper assumes that every Element is rendered in sequence from top to bottom. If styled in such a way that they are rendered from left to right, then you can set this prop to `true`. React-Paper will not render the elements horizontally, you have to do this with your own styles. All the library will do is overwrite every `<Node />` to have `content` of a type `'text'` and allocate space taking the largest Element into account. Defaults to `false`.

## Node

Can be used recursively in combination with `<Level />` components to create more complex layouts.
